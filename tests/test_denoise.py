...
import pandas as pd
import numpy as np
from src.steps.denoise import apply_transcript_denoise, AVAILABLE_DENOISE_BACKENDS


class TestApplyTranscriptDenoise:
    def test_backend_intracellular_filters_non_cellcomp(self, sample_transcripts_df):
        filtered, summary = apply_transcript_denoise(sample_transcripts_df, "intracellular")
    ...
    def test_backend_none_passes_through(self, sample_transcripts_df):
        filtered, summary = apply_transcript_denoise(sample_transcripts_df, "none")
        assert len(filtered) == len(sample_transcripts_df)
    ...
    def test_backend_nuclear_only(self, sample_transcripts_df):
        filtered, summary = apply_transcript_denoise(sample_transcripts_df, "nuclear_only")
    ...
    def test_drop_ratio_computed(self, sample_transcripts_df):
        _, summary = apply_transcript_denoise(sample_transcripts_df, "nuclear_only")
        assert "drop_ratio" in summary
        assert 0.0 <= summary["drop_ratio"] <= 1.0

    def test_empty_dataframe(self):
        df_empty = pd.DataFrame(columns=["CellComp", "fov", "cell_ID", "x_global_px", "y_global_px", "x_local_px", "y_local_px", "z", "target"])
        filtered, summary = apply_transcript_denoise(df_empty, "intracellular")
        assert len(filtered) == 0
        assert summary["drop_ratio"] == 0.0

    def test_available_backends(self):
        assert "none" in AVAILABLE_DENOISE_BACKENDS
        assert "intracellular" in AVAILABLE_DENOISE_BACKENDS
        assert "nuclear_only" in AVAILABLE_DENOISE_BACKENDS