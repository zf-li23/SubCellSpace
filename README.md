...
2. 执行可替换的 transcript 去噪后端（`none` / `intracellular` / `nuclear_only` / `sparc`）。
3. 执行可替换的细胞分割后端（`provided_cells` / `fov_cell_id`）。
4. 聚合为 cell-level AnnData。
5. 执行可替换的空间域识别后端（`spatial_leiden` / `spatial_kmeans` / `graphst` / `stagate` / `spagcn`）。
6. 执行可替换的亚细胞空间域聚类后端（`hdbscan` / `dbscan` / `leiden_spatial` / `phenograph` / `none`）。
7. 执行可替换的聚类分析后端（`leiden` / `kmeans` / `scvi`）。
8. 执行可替换的细胞类型注释后端（`cluster_label` / `rank_marker` / `celltypist`）。
9. 使用 Squidpy 构建空间邻接图，并封装到 SpatialData。

## 开发环境

- 建议使用 conda 环境 `subcellspace`。
...
### 亚细胞空间域 (Subcellular Spatial Domain)
| 后端 | 说明 |
|------|------|
